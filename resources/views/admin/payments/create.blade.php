...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.payments.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('payment_type') ? 'is-invalid' : '' }}" name="payment_type" id="payment_type" required>
                    ...
                    @foreach(App\Models\Payment::PAYMENT_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('payment_type', 'Bkash Merchant') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label class="required" for="number">{{ trans('cruds.payment.fields.number') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.payment.fields.number_helper') }}</span>
            </div>
            <div class="form-group">
