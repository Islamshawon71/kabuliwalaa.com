    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.cities.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('courier_name') ? 'is-invalid' : '' }}" name="courier_name_id" id="courier_name_id" required>
                    @foreach($courier_names as $id => $entry)
                        <option value="{{ $id }}" {{ old('courier_name_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('extras') ? 'is-invalid' : '' }}" type="text" name="extras" id="extras" value="{{ old('extras', '') }}">
