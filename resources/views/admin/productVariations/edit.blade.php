    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.product-variations.update", [$productVariation->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('product_variation_type') ? 'is-invalid' : '' }}" name="product_variation_type" id="product_variation_type" required>
                    ...
                    @foreach(App\Models\ProductVariation::PRODUCT_VARIATION_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('product_variation_type', $productVariation->product_variation_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('content') ? 'is-invalid' : '' }}" type="text" name="content" id="content" value="{{ old('content', $productVariation->content) }}" required>
