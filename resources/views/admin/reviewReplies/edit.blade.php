    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.review-replies.update", [$reviewReply->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('reply') ? 'is-invalid' : '' }}" type="text" name="reply" id="reply" value="{{ old('reply', $reviewReply->reply) }}" required>
                @if($errors->has('reply'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\ReviewReply::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $reviewReply->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
